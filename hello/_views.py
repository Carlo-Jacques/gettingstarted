...
def send_email(request):
    """
    Accepts POST from #formSelection and emails the payload to you.
    Works with standard form POST (application/x-www-form-urlencoded).
    """
    # Support both regular form POST and JSON body
    ...
    to_email = data.get("email")  # assumes your form has <input name="email">
    if not to_email:
        return JsonResponse({"ok": False, "error": "Missing email"}, status=400)

    # Build a simple message from all submitted fields
    lines = [f"{k}: {v}" for k, v in sorted(data.items())]
    body = "New submission from formSelection:\n\n" + "\n".join(lines)
    ...
    try:
        msg = EmailMessage(
            ...
            to=["carlo@charlottesweb.design"],                # send to the user
            bcc=[settings.EMAIL_HOST_USER]  # and bcc yourself (optional)
        )
        msg.send()
